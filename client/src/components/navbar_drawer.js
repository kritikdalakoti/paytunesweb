...
export default function TemporaryDrawer() {
	...
	useEffect(() => {
		// console.log('effect')
		fetch('/advertiser/get_advertisers', {
			//http://127.0.0.1:5000
			method: 'post'
		})
			.then((res) => res.json())
		...
		<div>
			...
			<div onClick={() => setopen(true)}>
				...
				<i
					...
					style={{ fontSize: '60px', cursor: 'pointer', color: 'grey', paddingLeft: '70%' }}
				>
					<ViewHeadlineIcon />
				</i>
			</div>
			<Tooltip title="Advertisers" placement="right">
				<div onClick={() => history.push('/advertisers')}>
					{' '}
					<i
						className="material-icons"
						style={{ fontSize: '60px', cursor: 'pointer', color: 'grey', paddingLeft: '70%' }}
			...
			<Tooltip title={`${adv.name}`} placement="right">
				<div onClick={() => handleOpen()}>
					{' '}
					<i
						className="material-icons"
						style={{ fontSize: '25px', cursor: 'pointer', color: 'grey', paddingLeft: '70%' }}
					>
						{adv.name[0].toLocaleUpperCase()}{' '}
					</i>
				</div>
			</Tooltip>
			...
			<Tooltip title="Creatives" placement="right">
				<div onClick={() => history.push('/creative')}>
					<i
						className="material-icons"
						style={{ fontSize: '60px', cursor: 'pointer', color: 'grey', paddingLeft: '70%' }}
					>
				...
				<div>
					...
					>
						<div style={{ maxHeight: '120vh' }} className={classes.paper}>
							...
							>
								{advertisers.map((adv) => <MenuItem value={adv}>{adv.name}</MenuItem>)}
							</Select>
						</div>
					</Modal>
				</div>
