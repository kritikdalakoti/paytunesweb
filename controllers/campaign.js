...
const fs=require('fs')
const {uploadAws,uploadMedia}=require('../aws_upload/uploadsfunction');
...
exports.createsubcampaign = async (req, res) => {
    try {
        ...
        if(req.files.AudioFileinp){
            ...
            fs.readFile(`./client/public/uploads/${req.files.AudioFileinp[0].filename}`, function(err,data){
                if(err){
                    throw new err;
                }
                let encoded=Buffer.from(data).toString('base64')
                let file=`audio/${req.files.AudioFileinp[0].filename}_${Date.now()}.${req.files.AudioFileinp[0].mimetype}`
                let data1={Key:file,Body:encoded,ContentEncoding: 'base64',ContentType: req.files.AudioFileinp[0].mimetype}
                uploadAws(data1)
                // console.log(encoded)
            })
        }
        if(req.files.VideoFileinp){
            ...
            fs.readFile(`./client/public/uploads/${req.files.VideoFileinp[0].filename}`, function(err,data){
                if(err){
                    throw new err;
                }
                let encoded=Buffer.from(data).toString('base64')
                let file=`video/${req.files.VideoFileinp[0].filename}_${Date.now()}.${req.files.VideoFileinp[0].mimetype}`
                let data1={Key:file,Body:encoded,ContentEncoding: 'base64',ContentType: req.files.VideoFileinp[0].mimetype}
                uploadAws(data1)
                // console.log(encoded)
            })
        }
        if(req.files.DisplayFileinp){
            fs.readFile(`./client/public/uploads/${req.files.DisplayFileinp[0].filename}`, async function  (err,data){
                ...
                console.log(encoded.length)
                 let filename= 'testing'//`${req.files.DisplayFileinp[0].filename}_${Date.now()}.${req.files.DisplayFileinp[0].mimetype}`
                let data1={filename,encoded}
                // console.log('ss',data1)
                await uploadMedia(data1)
                // console.log(encoded)
                console.log('hjj')
            })
        }
        ...
        if(req.files.AudioFileBanner){
            fs.readFile(`./client/public/uploads/${req.files.AudioFileBanner[0].filename}`, async function  (err,data){
                ...
                let encoded=Buffer.from(data).toString('base64')
                console.log(encoded.length)
                let filename=`${req.files.AudioFileBanner[0].filename}_${Date.now()}.${req.files.AudioFileBanner[0].mimetype}`
                let data1={filename,encoded}
                // console.log('ss',data1)
                await uploadMedia(data1)
                // console.log(encoded)
        ...
        if(req.files.VideoFileBanner){
            fs.readFile(`./client/public/uploads/${req.files.VideoFileBanner[0].filename}`, async function  (err,data){
                ...
                let encoded=Buffer.from(data).toString('base64')
                console.log(encoded.length)
                let filename=`${req.files.VideoFileBanner[0].filename}_${Date.now()}.${req.files.VideoFileBanner[0].mimetype}`
                let data1={filename,encoded}
                // console.log('ss',data1)
                await uploadMedia(data1)
                // console.log(encoded)
        ...
        if(req.files.DisplayFileBanner){
            fs.readFile(`./client/public/uploads/${req.files.DisplayFileBanner[0].filename}`, async function  (err,data){
                ...
                let encoded=Buffer.from(data).toString('base64')
                console.log(encoded.length)
                let filename=`${req.files.DisplayFileBanner[0].filename}_${Date.now()}.${req.files.DisplayFileBanner[0].mimetype}`
                let data1={filename,encoded}
                // console.log('ss',data1)
                await uploadMedia(data1)
                // console.log(encoded)
